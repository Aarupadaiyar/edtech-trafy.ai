...
export default function Sitemap() {
  return (
    <section className="mx-auto max-w-4xl px-6 pt-32 pb-24">
      ...
      <p className="mt-4 text-lg text-white/50">A complete overview of all pages and resources on Trafy.</p>

      <div className="mt-14 grid gap-10 sm:grid-cols-2 lg:grid-cols-3">
        {sections.map((section) => (
          <div key={section.title}>
            <h2 className="font-display text-xs font-bold uppercase tracking-widest text-white/30">{section.title}</h2>
              ...
              {section.links.map((link) => (
                <li key={link.label}>
                  {"to" in link ? (
                    ...
                    <a href={link.href} target="_blank" rel="noreferrer" className="group inline-flex items-center gap-1 text-sm text-white/60 transition-colors hover:text-white">
                      {link.label}
