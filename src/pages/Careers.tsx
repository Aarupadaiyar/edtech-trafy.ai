...
export default function Careers() {
  return (
    <section className="mx-auto max-w-4xl px-6 pt-32 pb-24">
      ...
      {/* Perks */}
      <div className="mt-16">
        <h2 className="font-display text-xs font-bold uppercase tracking-widest text-white/30">Why Join Trafy</h2>
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {perks.map((p) => (
            <div key={p.title} className="rounded-2xl border border-white/10 bg-white/5 p-5 transition-all duration-300 hover:border-white/20">
      ...
      {/* Openings */}
      <div className="mt-20">
        <h2 className="font-display text-xs font-bold uppercase tracking-widest text-white/30">Open Positions</h2>
        <div className="mt-8 space-y-3">
          {openings.map((job) => (
            <a
              key={job.title}
              href={`mailto:careers@trafy.ai?subject=Application: ${job.title}`}
              className="group flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 px-6 py-5 transition-all duration-300 hover:border-white/20 hover:shadow-md"
            >
              <div>
              ...
              <span className="text-sm font-semibold text-white/30 transition-colors group-hover:text-green-hard">
                Apply →
              </span>
            </a>
          ))}
