...
function createTextTexture(gl, text, font = 'bold 30px monospace', color = 'black') {
  ...
  const textHeight = Math.ceil(30 * 1.2);
...
class Title {
  constructor({ gl, plane, text, textColor = '#eeeeee', font = "600 22px 'JetBrains Mono'" }) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
    this.text = text;
...
class App {
  constructor(container, { items, bend, textColor = '#eeeeee', borderRadius = 0, font = "600 22px 'JetBrains Mono'", scrollSpeed = 2, scrollEase = 0.05, onActiveChange, onItemSelect } = {}) {
  ...
  onTouchDown(e) { this.isDown = true; this.scroll.position = this.scroll.current; this.start = e.touches ? e.touches[0].clientX : e.clientX; this.moved = false; }
  onTouchMove(e) {
    if (!this.isDown) return;
    const x = e.touches ? e.touches[0].clientX : e.clientX;
    const distance = (this.start - x) * (this.scrollSpeed * 0.025);
    if (Math.abs(distance) > 2) this.moved = true;
  ...
  onTouchUp(e) {
    ...
    if (!this.moved && this.onItemSelect) {
      this.onItemSelect(this.activeIndex ?? 0);
    }
  }
  ...
  update() {
    ...
    if (this.medias) {
      ...
      this.medias.forEach(media => {
        ...
        if (d < minDist) { minDist = d; nearest = media.index % (this.rawItems?.length || 1); }
      });
      if (nearest !== this.activeIndex) {
        this.activeIndex = nearest;
        if (this.onActiveChange) this.onActiveChange(nearest);
      }
    }
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const app = new App(containerRef.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, onActiveChange, onItemSelect });
    return () => app.destroy();
  }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase]);
  return <div className="circular-gallery" ref={containerRef} tabIndex={0} role="region" aria-label="Project gallery. Drag to browse, click a project to see details." />;
}
